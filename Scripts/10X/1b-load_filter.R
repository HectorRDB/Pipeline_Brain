...
option_list <- list(
  ...
  make_option(c("-m", "--meta"),
                action = "store", default = NA, type = "character",
                help = "Where to store the metadata output"
  ),
...
write_csv(x = counts, path = output)
write_csv(x = meta %>% select(Samples, sequencing_tube_barcode),
          path = paste0(output, "_batches.csv"))
write_csv(x = meta %>% select(Samples, allenClusters),
          path = paste0(output, "_labels.csv"))