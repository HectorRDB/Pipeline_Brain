...
.libPaths(c("/pylon5/ib5phhp/diyadas/rpack/3.5", .libPaths()))
cat(.libPaths(), "\n")
library(stringr)
...
cat("Saving output at ", output_n, "\n")
saveRDS(zinb0, file = output_n)
...
zinbWs <- lapply(zinbDims, function(zinbDim) {
  ...
  loc <- str_replace(output_r, "\\.rds", paste0("_", zinbDim, ".rds"))
  saveRDS(zinb, file = loc)
  cat("Saving output at ", output_r, "\n")
  return(zinb)
})
