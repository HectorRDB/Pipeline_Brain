...
create_replicability_tsnes = function(result_path = "../mn_results/smart_tenx", output_dir = "../tsne", data_path = "../../data") {
...
load_tsne_coord = function(cell_names, data_path) {
    tsne_coord = bind_rows(
        zeng_smart_cells = read.csv(file.path(data_path, "Smart-Seq/SMARTer_cells_MOp_tnse.csv")),
        zeng_smart_nuclei = read.csv(file.path(data_path, "Smart-Seq/SMARTer_nuclei_MOp_tnse.csv")),
        zeng_10x_cells = read.csv(file.path(data_path, "10X/10x_cells_MOp_tnse.csv")),
        zeng_10x_nuclei = read.csv(file.path(data_path, "10X/10x_nuclei_MOp_tnse.csv")),
        .id = "dataset"
    )
...
compute_replicability_matrix = function(label_matrix, output_dir) {
    ...
    is_replicable = lapply(set_names(labels), function(l) find_replicable_cluster(file.path(output_dir, l)))
...
read_component_summary = function(results_dir) {
    read.table(file.path(results_dir, "component_summary.txt"), sep = "\t", header = TRUE, stringsAsFactors = FALSE)
}
...
plot_scores = function(scores, tsne_coord, dataset_name, merging_step_name, dot_size = 0.01) {
    scores %>%
