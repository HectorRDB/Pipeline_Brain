...
main_full_data = function(result_path = "../mn_results", output_dir = "..") {
    ...
    create_summary_figures(label_matrix, file.path(result_path, "smart"),
                           file.path(output_dir, "smart"), 2, exclude_rsec = FALSE)
    create_summary_figures(label_matrix, file.path(result_path, "tenx"), file.path(output_dir, "tenx"), 2)
    create_summary_figures(label_matrix, file.path(result_path, "cells"), file.path(output_dir, "cells"), 2)
    create_summary_figures(label_matrix, file.path(result_path, "nuclei"), file.path(output_dir, "nuclei"), 2)
}
    
create_summary_figures = function(label_matrix, result_path, output_dir, n_datasets, exclude_rsec = TRUE) {
    ...
    ggsave(file.path(output_dir, "fraction_mapping_clusters.pdf"), plot_fraction_mapped(results))
    ggsave(file.path(output_dir, "number_mapping_clusters.pdf"), plot_number_mapped(results, n_datasets))
    ggsave(file.path(output_dir, "fraction_mapping_cells.pdf"), plot_mapped_cells(results))
    ggsave(file.path(output_dir, "mapping_quality.pdf"), plot_mapping_quality(score))
    ggsave(file.path(output_dir, "fraction_replicable_vs_number_clusters.pdf"), plot_clusters_vs_mapped(results))

    pdf(file.path(output_dir, "merging_process.pdf"))
    grid::grid.newpage()
    grid::grid.draw(
        rbind(ggplotGrob(plot_number_total_2(results, n_datasets)), ggplotGrob(plot_mapped_cells_2(results)), size = "last")
    )
    dev.off()
}
...
get_results = function(result_path, label_matrix) {
    ...
    for (l in method_labels) {
        ...
        mapped_cells = c(mapped_cells, number_mapped_cells(components, cluster_size[[l]]))
        unmapped_cells = c(unmapped_cells, number_unmapped_cells(components, cluster_size[[l]]))
        outliers = c(outliers, number_outliers(components))
        score[[l]] = score_components(best_hits, components$components, full_name = TRUE)$score
    }    
    ...
    score = score %>%
        mutate(clustering_name = sapply(strsplit(as.character(clustering_method), split = ".", fixed = TRUE), "[", 1)) %>%
        mutate(level = sapply(strsplit(as.character(clustering_method), split = ".", fixed = TRUE), "[", 2))
    ...
    results = results %>%
        mutate(fraction_replicable_clusters = replicable_clusters / (replicable_clusters + non_replicable_clusters)) %>%
        mutate(fraction_replicable_cells = replicable_cells / (replicable_cells + non_replicable_cells)) %>%
        mutate(clustering_name = sapply(strsplit(as.character(clustering_method), split = ".", fixed = TRUE), "[", 1)) %>%
        mutate(level = sapply(strsplit(as.character(clustering_method), split = ".", fixed = TRUE), "[", 2))

    if (any(results$level %in% c("Initial", "Final"))) {
        results$level = factor(results$level, levels = c("Initial", "33", "66", "90", "Final"))
    } else {
...
read_components = function(result_path) {
    components = read.table(file.path(result_path, "component_summary.txt"), sep = "\t", header = TRUE, stringsAsFactors = FALSE)
    comp = lapply(unique(components$Component), function(c) components$Label[components$Component == c])
...
read_best_hits = function(result_path) {
    result = read.table(file.path(result_path, "best_hits.txt"), check.names = FALSE, stringsAsFactors = FALSE)
...
plot_fraction_mapped = function(results) {
    ggplot(results, aes(x = level, y = fraction_replicable_clusters, group = clustering_name, col = clustering_name)) +
        geom_line() +
        geom_point() +
        theme_bw() +
...
plot_number_mapped = function(results, n_datasets) {
    ggplot(results, aes(x = level, y = replicable_clusters / n_datasets, group = clustering_name, col = clustering_name)) +
        geom_line() +
        geom_point() +
        theme_bw() +
        theme(legend.position = "top", axis.ticks.x = element_blank(), axis.text.x = element_blank()) +
...
plot_mapped_cells = function(results) {
    ggplot(results, aes(x = level, y = fraction_replicable_cells,
...
plot_number_total = function(results, n_datasets) {
    ggplot(results, aes(x = level, y = (replicable_clusters + non_replicable_clusters) / n_datasets,
                        group = clustering_name, col = clustering_name)) +
        geom_line() +
        geom_point() +
        theme_bw() +
...
plot_mapped_cells_2 = function(results) {
    ggplot(results, aes(x = level, y = fraction_replicable_cells,
        ...
        geom_text(data = subset(results, as.numeric(level) == 1),
...
plot_clusters_vs_mapped = function(results) {
    results %>%
        arrange(level) %>%
        ggplot(aes(x = replicable_clusters + non_replicable_clusters,
