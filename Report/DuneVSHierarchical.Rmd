...
```{r laod data}
singleMethod <- read.table(here("data", "Replicability", "singleMethod", "smart",
  ...
  mutate(
    ...
    merging_type = "Changing the parameters")
```
...
```{r}
Seurat <- Seurat %>% mutate(level = as.numeric(as.character(level)))
Seurat_k <- Seurat %>%
  filter(clustering_method <= 1.2 & level == 50)
  
df <- bind_rows(SC3, Monocle %>% filter(level >= 45), Seurat_k,
