...
for (i in 1:3) {
  cellsConsensus <- suppressWarnings(
    makeConsensus(x = clusters, clusterLabel = "makeConsensus", 
                  proportion = prop[i], minSize = 20)
                  )
  consensus <- cellsConsensus$clustering
  inds <- consensus != -1
  ARIs[i] <- adjustedRandIndex(consensus[inds],
  ...
  title(paste("Consensus Merging with proportion ", round(prop[i], 2)))
}

ARIs %>% kable()
rm(ARIs, prop, clusters, cellsConsensus, consensus, i, inds)
```
